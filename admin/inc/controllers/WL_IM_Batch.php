		...
		if ( count( $data ) !== 0 ) {
			foreach ( $data as $row ) {
				...
				$results["data"][] = array(
					...
					esc_html($added_on),
					esc_html($added_by),
					'<a class="mr-3" href="#update-batch" data-keyboard="false" data-backdrop="static" data-toggle="modal" data-id="' . esc_attr($id) . '"><i class="fa fa-edit"></i></a> <a href="javascript:void(0)" delete-batch-security="' . wp_create_nonce( "delete-batch-". esc_attr($id) ) . '"delete-batch-id="' . esc_attr($id). '" class="delete-batch"> <i class="fa fa-trash text-danger"></i></a>'
				);
			}
		} else {
			$results["data"] = [];
	...
	public static function update_batch() {
		...
		if ( count( $errors ) < 1 ) {
			try {
				...
				$wpdb->query( 'COMMIT;' );
				wp_send_json_success( array( 'message' => esc_html__( 'Batch updated successfully.', WL_IM_DOMAIN ) ) );
			} catch ( Exception $exception ) {
		  		$wpdb->query( 'ROLLBACK;' );
				wp_send_json_error( $exception->getMessage() );
			}
