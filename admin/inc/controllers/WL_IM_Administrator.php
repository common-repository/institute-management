...
class WL_IM_Administrator {
		...
		if ( count( $data ) !== 0 ) {
			foreach ( $data as $row ) {
				...
				$results["data"][] = array(
					...
					esc_attr($last_name),
					esc_attr($username),
					wp_kses_post($permissions),
					esc_attr($added_on),
					'<a class="mr-3" href="#update-administrator" data-keyboard="false" data-backdrop="static" data-toggle="modal" data-id="' . esc_attr($id) . '"><i class="fa fa-edit"></i></a>'
				);
			}
		} else {
			$results["data"] = [];
	...
	public static function fetch_administrator() {
		...
		<form id="wlim-update-administrator-form">
			...
			<div class="row">
				<div class="col form-group">
					<label for="wlim-administrator-phone_update" class="col-form-label"><?php esc_html_e( 'Phone', WL_IM_DOMAIN ); ?>:</label>
					<input name="phone" type="text" class="form-control" id="wlim-administrator-phone_update" placeholder="<?php esc_attr_e( "Phone", WL_IM_DOMAIN ); ?>" value="<?php echo get_user_meta( $id, 'phone', true ); ?>">
				</div>
				<div class="col form-group">
					<label for="wlim-administrator-email_update" class="col-form-label"><?php esc_html_e( 'Email', WL_IM_DOMAIN ); ?>:</label>
					<input name="email" type="text" class="form-control" id="wlim-administrator-email_update" placeholder="<?php esc_attr_e( "Email", WL_IM_DOMAIN ); ?>" value="<?php echo get_user_meta( $id, 'email', true ); ?>">
				</div>
			</div>
			...
			<div class="form-check pl-0">
				<input name="permissions[]" class="position-static mt-0 form-check-input" type="checkbox" id="<?php echo esc_attr($capability_key)."_update"; ?>" value="<?php echo esc_attr($capability_key); ?>" <?php echo user_can( $row->ID, WL_IM_Helper::$core_capability ) ? 'disabled' : '' ?>>
				<label class="form-check-label" for="<?php echo esc_attr($capability_key)."_update"; ?>"><?php esc_html_e( $capability_value, WL_IM_DOMAIN ); ?></label>
			</div>
			<?php
			} ?>
		...
		<script>
			...
			foreach( $permissions as $capability ) { ?>
			jQuery("#<?php echo esc_attr($capability)."_update"; ?>").prop('checked', true);
			<?php
			} ?>
		</script>
	<?php
