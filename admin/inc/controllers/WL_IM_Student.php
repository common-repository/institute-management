...
class WL_IM_Student {
		...
		if ( count( $data ) !== 0 ) {
			foreach ( $data as $row ) {
				...
				if ( $row->course_id && isset( $course_data[$row->course_id] ) ) {
					...
					if ( $course_completed ) {
						$duration .= `<span class="text-primary"><strong class="text-primary">` . esc_html__( ' Completed', WL_IM_DOMAIN ) . `</strong></span>`;									
					}
				}
				...
				$results["data"][] = array(
					...
					'<a class="mr-3" href="#update-student" data-keyboard="false" data-backdrop="static" data-toggle="modal" data-id="' . esc_attr($id) . '"><i class="fa fa-edit"></i></a> <a href="javascript:void(0)" delete-student-security="' . wp_create_nonce( "delete-student-" .esc_attr($id) ) . '"delete-student-id="' . esc_attr($id). '" class="delete-student"> <i class="fa fa-trash text-danger"></i></a>'
				);
			}
		} else {
			$results["data"] = [];
	...
	public static function fetch_course_update_batches() {
		...
		<script>
			...
			<?php
			if ( $batch_id ) { ?>
			jQuery('#wlim-student-batch_update').selectpicker('val', '<?php echo esc_attr($batch_id); ?>');
			<?php
			} ?>
		</script>
		<?php
	...
	public static function fetch_enquiry() {
		...
		<div class="form-group">
			...
			<input name="phone" type="text" class="form-control" id="wlim-student-phone" placeholder="<?php esc_attr_e( "Phone", WL_IM_DOMAIN ); ?>" value="<?php echo esc_attr($row->phone); ?>">
		</div>
		<div class="form-group">
		...
		<div class="form-group">
			...
			<input name="email" type="email" class="form-control" id="wlim-student-email" placeholder="<?php esc_attr_e( "Email", WL_IM_DOMAIN ); ?>" value="<?php echo esc_attr($row->email); ?>">
		</div>
		<div class="form-check pl-0">
		...
		<script>
			...
			jQuery('#wlim-student-course').selectpicker('val', '<?php echo esc_attr($row->course_id); ?>');
	...
	public static function fetch_fees_payable() {
		...
		<div class="form-group">
			<label for="wlim-student-fees_payable" class="col-form-label"><?php esc_html_e( 'Fees Payable', WL_IM_DOMAIN ); ?>:</label>
			<input name="fees_payable" type="number" class="form-control" id="wlim-student-fees_payable" min="0" placeholder="<?php esc_attr_e( "Fees Payable", WL_IM_DOMAIN ); ?>" value="<?php echo esc_attr($row->fees); ?>">
		</div>
		<?php
        } else { ?>
		<div class="form-group">
