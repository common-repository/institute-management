		...
		if ( count( $data ) !== 0 ) {
			foreach ( $data as $row ) {
				...
				$results["data"][] = array(
					...
					esc_html($duration) . esc_html($duration_in),
					esc_html($fees),
					esc_html($is_acitve),
					esc_html($added_on),
					esc_html($added_by),
					'<a class="mr-3" href="#update-course" data-keyboard="false" data-backdrop="static" data-toggle="modal" data-id="' . esc_attr($id). '"><i class="fa fa-edit"></i></a> <a href="javascript:void(0)" delete-course-security="' . wp_create_nonce( "delete-course-". esc_attr($id) ) . '"delete-course-id="' . esc_attr($id). '" class="delete-course"> <i class="fa fa-trash text-danger"></i></a>'
				);
			}
		} else {
			$results["data"] = [];
