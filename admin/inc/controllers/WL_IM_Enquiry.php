...
class WL_IM_Enquiry {
		...
		if ( count( $data ) !== 0 ) {
			foreach ( $data as $row ) {
				...
				$results["data"][] = array(
					...
					esc_html($added_by),
					esc_html($date),
					'<a class="mr-3" href="#update-enquiry" data-keyboard="false" data-backdrop="static" data-toggle="modal" data-id="' . esc_attr($id) . '"><i class="fa fa-edit"></i></a> <a href="javascript:void(0)" delete-enquiry-security="' . wp_create_nonce( "delete-enquiry-".  esc_attr($id) ) . '"delete-enquiry-id="' . esc_attr($id). '" class="delete-enquiry"> <i class="fa fa-trash text-danger"></i></a>'
				);
			}
		} else {
			$results["data"] = [];
	...
	public static function add_enquiry() {
		...
		$mail = $wpdb->get_var( "SELECT email FROM {$wpdb->prefix}wl_im_enquiries WHERE email = '$email' AND is_deleted = 0" );
		...
		if ( $mail === $email ) {
			$errors['email'] = esc_html__( 'Email address already exist.', WL_IM_DOMAIN );
		}
